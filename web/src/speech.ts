...
export function makeSpeech(
  ...
  const sentences = makeSentences(opts)
...
function makeSentences(
  ...
  return sentences.map<Sentence>((text, i) => ({
    ...
    getPcmData: lazy(() => synthesize(text, opts.voiceId))
  }))
}
...
async function prefetch(
  ...
  let numSecondsToPrefetch = config.numSecondsToPrefetch

  //wait until the current phrase has been synthesized before prefetching
  await sentences[sentenceIndex].getPcmData()
  await wait(playbackState, "resumed")

  while (numSecondsToPrefetch > 0 && sentenceIndex + 1 < sentences.length) {
    ...
    numSecondsToPrefetch -= pcmData.samples.length / pcmData.sampleRate
  }
}
